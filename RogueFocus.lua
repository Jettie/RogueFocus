...
function RogueFocus:OnUpdate()
	if(RogueFocus.Registered) then
		...
		if(energy > RogueFocus_Energy.Mana or currentTime >= RogueFocus_Energy.End) then
			...
			sparkPosition = ((currentTime - RogueFocus_Energy.Start) / (RogueFocus_Energy.End - RogueFocus_Energy.Start)) * 98
		end
		--调整tick的丝滑感受。修改为0-1之间的比值
		local fraction = ( currentTime - RogueFocus_Energy.Start ) / RogueFocus_Energy.Length
		RogueFocusEnergyTickBar:SetMinMaxValues(0 , 1)
		RogueFocusEnergyTickBar:SetValue(fraction)
